...
describe('adapter-digital-ocean', () => {
  ...
  describe('ensureRateLimitNotExceeded', () => {
    it('throws error if status code is 429 (Quota exceeded)', () => {
      const response: Partial<AxiosResponse> = {
        status: 429,
        data: {
          id: 'error-id',
      ...
      const fn = (dnsProvider as any).ensureRateLimitNotExceeded;
      expect(() => fn(response)).toThrow(ForbiddenError);
    });

    it('returns undefined (void) if the status is anything other than 429 (Quota exceeded)', () => {
      const response: Partial<AxiosResponse> = {
        status: 402,
      };
  ...
  describe('fetchDomainRecords', () => {
    it("returns the requested domain's records", async () => {
      ...
      expect(url).toBe(`/domains/${DOMAIN_NAME}/records`);
      ...
      expect(result).toStrictEqual(
        FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records
      );
    });
  });
  ...
  describe('findDomainRecord', () => {
    it('returns the domain record', () => {
      const result: FindDomainRecordResult = (
        ...
        domainRecords: FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records,
        filters: {
          recordType: DnsRecordType.A,
          recordName: 'a',
        },
      ...
      expect(typeof result).toBe('object');
      expect(result).toStrictEqual(
        FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records[0]
      );
    });
  ...
  describe('createDomainRecord', () => {
    it('returns the created record details', async () => {
      ...
      const result: CreateDomainRecordResult = await (
        ...
        httpClient: httpClient,
        domainName: DOMAIN_NAME,
        domainRecord: FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records[0],
      });
  ...
  describe('updateRecord', () => {
    ...
    it('always updates the record, no matter what', async () => {
      ...
      sinon.replace(
        dnsProvider as any,
        'fetchDomainRecords',
        async () => FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records
      );
      const createDomainRecordSpy = sinon.replace(
        dnsProvider as any,
        'createDomainRecord',
      ...
      const updateDomainRecordSpy = sinon.replace(
        dnsProvider as any,
        'updateDomainRecord',
        sinon.fake(() => FETCH_DOMAIN_RECORDS_MOCK_RESPONSE.domain_records[0])
      );
